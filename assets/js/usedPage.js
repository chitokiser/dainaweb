...
function resetForm(){
  ["area","tag","price","condition","title","desc","contact","href"].forEach(id=>{
...
function setFold(collapsed){
  ...
  try{
    localStorage.setItem("daina_used_fold", foldCollapsed ? "1" : "0");
  }catch{}
}
function toggleFold(){ setFold(!foldCollapsed); }
function openFold(){ setFold(false); }
...
function cardPublicHTML(item){
  ...
  const extra = [price && ("가격: " + price), cond && ("상태: " + cond)].filter(Boolean).join(" · ");

  return `
    <a class="card" href="${href}" role="article" onclick="${href==="#" ? "return false;" : ""}">
      <div class="card-top">
        <h3 class="card-title">${title}</h3>
        <span class="card-tag">${tag || "중고"}</span>
      </div>
      ...
      ${extra ? `<div class="card-meta"><span>${esc(extra)}</span><span></span></div>` : ""}
    </a>
  `;
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  try{
    const saved = localStorage.getItem("daina_used_fold");
    setFold(saved === "1");
  }catch{
    setFold(false);
  }
